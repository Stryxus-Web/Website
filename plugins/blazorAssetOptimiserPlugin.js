...
class BlazorAssetOptimiserPlugin {
    apply(compiler) {
        compiler.hooks.compilation.tap('BlazorAssetOptimiserPlugin', compilation => {
            compilation.hooks.processAssets.tap({
                name: 'BlazorAssetOptimiserPlugin',
                        ...
                        if (!caches.includes(sha1)) {
                            ...
                            for (var x = 1; x > 0; x -= 0.25) {
                                ...
                                for (var y = 0; y < 15; y++) {
                                    ...
                                    img.toBuffer({ resolveWithObject: true }, (err, buffer, info) => avif = buffer);
                                    checkForAVIFResults();
                                    if (avif.byteLength <= byteMaxSize) break;
                                }
                                if (avif.byteLength <= byteMaxSize) break;
                            }

                            img.webp({ quality: 100 - (qualityDecrement * 5), effort: process.env.NODE_ENV === 'production' ? 6 : 0 })
                            img.toBuffer({ resolveWithObject: true }, (err, buffer, info) => webp = buffer);
...
module.exports = BlazorAssetOptimiserPlugin;
